...
class MqttClient:
    client: Client = Client(client_id=f"raspberry", clean_session=True, protocol=MQTTv311, reconnect_on_failure=True)
    run_id: int = 0
    ...
    def connect(cls, runner_id: int, server_ip: str, port: int = 1883) -> None:
        ...
        cls.client._client_id = f"raspberry_{runner_id}"
    ...
    def publish_log(cls) -> None:
        ...
        MqttClient.client.publish(f"ice_runner/raspberry_pi/{cls.run_id}/log", str(MqttClient.run_logs))
    ...
    async def start(cls) -> None:
        """The function subscribe ServerMqttClient to commander topics and starts new thread to process network traffic"""
